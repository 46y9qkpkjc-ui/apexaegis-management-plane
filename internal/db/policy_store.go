...
func (s *PolicyStore) List() []policy.SecurityPolicy {
	rows, err := s.db.QueryContext(context.Background(),
		`SELECT id, org_id, name, sequence, enabled,
		        ...
		        log_traffic, log_security_events, source_device_posture, is_default
		 FROM system_mgmt.policies ORDER BY is_default, sequence`)
	if err != nil {
...
func (s *PolicyStore) upsertPolicy(p *policy.SecurityPolicy) error {
	...
	_, err := s.db.ExecContext(context.Background(),
		`INSERT INTO system_mgmt.policies (
			...
			log_traffic=$41, log_security_events=$42, source_device_posture=$43, is_default=$44`,
		...
		p.LogTraffic, p.LogSecurityEvents, p.SourceDevicePosture, p.IsDefault,
	)
	return err
}
