...
// The TCP listener is ALWAYS open (so NLB health checks pass and ECS keeps
// the task alive).  When the feature is disabled, every accepted connection
// is closed immediately — no TLS handshake, no goroutine, no proxy.  This
// drops DDoS traffic at near-zero cost while keeping the port reachable
// for health probes.
func (s *Server) Run(ctx context.Context) {
	...
	if err != nil {
		s.logger.Fatal("radsec: listen failed", zap.String("addr", s.cfg.ListenAddr), zap.Error(err))
	}
	defer ln.Close()
	s.logger.Info("radsec: TCP listener open", zap.String("addr", s.cfg.ListenAddr))

	if !s.isEnabled() {
		s.logger.Info("radsec: feature disabled at startup — connections will be dropped")
	}
	...
	for {
		...
		if !s.isEnabled() {
			// Feature disabled: close immediately (DDoS drop, no TLS).
			conn.Close()
			continue
		}
