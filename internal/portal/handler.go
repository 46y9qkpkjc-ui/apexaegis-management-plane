...
func (h *Handler) HandleDownloadPS1(c *gin.Context) {
	...
	script += "$agentMsi = \"$env:TEMP\\ApexAegis-Setup.msi\"\n"
	script += "try {\n"
	...
	script += "    $msiFile = ($latest.files | Where-Object { $_.filename -like '*.msi' } | Select-Object -First 1)\n"
	script += "    if (-not $msiFile) { throw \"No MSI found in latest release\" }\n"
	script += "    $downloadUrl = \"$drsEndpoint\" + $msiFile.path\n"
	script += "    Invoke-WebRequest -Uri $downloadUrl -OutFile $agentMsi -UseBasicParsing\n"
	script += "    Write-OK \"Agent downloaded ($($msiFile.filename))\"\n"
	script += "} catch {\n"
